...
namespace fgui {
	...
	public:
		...
		void add_item(const std::string_view item, const fgui::state &state) noexcept {

			m_info.push_back( { item, state } );
		}

		// select a specific item at a given position
		void set_selected(const int &index, const bool &onoff) noexcept {

			m_info[index].checked = onoff;
		}

		// get the item selected at a given position
		bool get_selected(int index) const noexcept {
