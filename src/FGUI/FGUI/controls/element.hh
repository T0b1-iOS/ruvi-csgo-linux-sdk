...
namespace fgui {
	...
	public:
		...
		void set_size(const int &width, const int &height) noexcept {
		...
		fgui::dimension get_size() const noexcept {

			return { m_width, m_height };
		}

		// get the current position of the element
		fgui::point get_position() const noexcept {
		...
		void set_family(const int &type) noexcept {

			m_type =  static_cast<int>(type);
		}

		// get the element type
		bool get_family(const fgui::detail::element_type &type) const noexcept {
		...
		void set_controller(const std::shared_ptr<fgui::element> &controller) noexcept {

			m_controller = controller;
		}

		// sets a custom indentificator for the element (this is used by the config system)
		void set_identificator(const std::string_view identificator) noexcept {

			m_identificator = identificator;
		}

		// get the current identificator of the element
		std::string_view get_identificator() const noexcept {

			return m_identificator;
		}

		// allows the user to change the font of the element
		void set_font(const std::string_view font_family, int size, int flags = 0, bool bold = false) noexcept {

			fgui::render.create_font(m_font, font_family, size, flags, bold);
		}

		void set_font(const fgui::element_font &element_font) noexcept {
		...
		fgui::font get_font() const noexcept {
