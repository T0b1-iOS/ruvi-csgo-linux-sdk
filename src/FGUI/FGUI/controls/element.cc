...
//---------------------------------------------------------
fgui::element* fgui::element::get_window() {

    fgui::element* current_container = this;

	while (current_container->m_parent_element)
		current_container = current_container->m_parent_element.get();

	return current_container;
}

//---------------------------------------------------------
fgui::element* fgui::element::get_first_container() {
	
    fgui::element* first_container = this;

	while (!first_container->get_family(fgui::detail::element_type::CONTAINER))
		first_container = first_container->m_parent_element.get();
...
fgui::point fgui::element::get_absolute_position() {
	...
	// element usable area 
	if (m_parent_element.get() == get_window())
		m_absolute_position.y += header_bar_size;
	
	else if (m_parent_element.get() != get_window())
